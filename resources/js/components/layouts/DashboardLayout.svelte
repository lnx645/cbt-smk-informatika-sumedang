<script lang="ts">
    ...
    import '@/styles/modules/dashboard-layout.scss';
...
<div class="d-flex min-vh-100 bg-light-subtle">
    ...
    >
        ...
        >
            ...
            <div class="mt-1 d-flex align-items-center gap-2">
                <span
                    class={`badge rounded-pill dashboard-sidebar-badge`}
                    >Tut Wuri</span
                >
        ...
        <nav class="px-3 d-flex flex-column gap-1">
            <span
                class={`sidebar-label small text-uppercase px-2 mb-1 sidebar-label`}
                >Menu Utama</span
            >
            {#each primaryLinks as link (link.label)}
                <Link
                    href={link.url}
                    class={`sidebar-link d-flex align-items-center gap-2 px-3 py-2 rounded text-white text-decoration-none ${onDashboard ? 'sidebar-link-active' : ''}`}
                >
                    {link.label}
                </Link>
            {/each}
            ...
            <span
                class={`sidebar-label small text-uppercase px-2 mt-3 mb-1 sidebar-label`}
                >Lainnya</span
            >
            {#each secondaryLinks as link (link.label)}
                {#if link.url}
                    <Link
                        href={link.url}
                        class={`sidebar-link d-flex align-items-center gap-2 px-3 py-2 rounded text-white opacity-75 text-decoration-none sidebar-link`}
                    >
                        {link.label}
                    </Link>
                {:else}
                    <span
                        class={`sidebar-link d-flex align-items-center gap-2 px-3 py-2 rounded text-white opacity-50 sidebar-link`}
                    >
                        {link.label}
                    </span>
                {/if}
    ...
    <div class="d-flex flex-column flex-grow-1" style="min-width: 0">
        ...
        >
            ...
            <span
                class={`badge rounded-pill text-dark dashboard-sidebar-badge`}
                >Admin</span
            >
        </header>
